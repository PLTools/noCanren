...
let env_free_vars env =
  let rec get_fvs env =
    ...
    | []        -> []
    | p :: env' ->
      match p with v, gtyp -> append (gen_free_vars gtyp) (get_fvs env')
  in remove_repeats (get_fvs env)
...
let rec lookup_total v s =
  ...
  | p :: xs ->
    match p with v', t ->
      if v = v' then Some t else lookup_total v xs
...
let env_apply s env =
  let member_apply p =
    match p with v, gtyp -> v, gen_apply s gtyp
  in map member_apply env
...
let compose_subst s1 s2 =
  let apply_member p =
    match p with
    | k, v -> k, apply s1 v
  in append (map apply_member s2) s1

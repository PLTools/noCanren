...
let skip_bindings = {
  ...
  expr = (fun self e ->
    ...
    | Texp_let (_, [{vb_attributes}], body) when Util.has_named_attribute "only_lozovml" vb_attributes ->
        self.expr self body
  ...
  let rec untype_item = function
    | Types.Sig_value (id, vd, _) ->
      ...
      | Types.Sig_type (id, tdecl, is_rec, _) ->
        Sig.type_ Nonrecursive (List.map (fun f -> f (Location.mknoloc (Ident.name id))) (untype_tdecl tdecl) )
    | _ -> assert false
  and untype_tdecl decl =
      let manifest = match decl.type_manifest with
      | None -> assert false
      | Some t -> untype_type t
    in
    let params = (List.map (fun t -> (untype_type t, (Asttypes.NoVariance, Asttypes.NoInjectivity))) decl.type_params ) in
...
let untype_module_type mt = match mt.mty_desc with
  | Tmty_ident (_, {txt=Lident ident; loc}) -> Ast_helper.Mty.ident (Location.mkloc (Lident ident) loc)
  | _ -> assert false

...
let inj: ('a, 'b) fancy -> ('a, 'b logic) fancy = fun x -> x
(* let injlift : 'a -> 'a fancy = fun x -> x;; *)
...
let fancy = {
 ...
 GT.plugins =
   object
     ...
     method html f a = f a
     method eq      f a = f a
     method compare f a = f a
     method foldl   f a = f a
     method foldr   f a = f a
     method show f a = f a
   end
};;
...
(* helps to refine ('a,'b) fancy to 'b *)
(* let refine_fancy: ('a, 'b) fancy -> var_checker -> ('a fancy -> 'c logic) -> 'c logic =
  fun x c refiner ->
  if c#isVar x
  then match !!!x with
  | InnerVar (_,_, i, cs) -> Var (i, List.map (fun x -> Obj.magic @@ refiner !!!x) cs)
...
let refiner : ('a,'b) fancy -> ('a,'b) fancy refiner = fun x ans ->
  let f ((e,_,_) as st) =
    let ans = refine st x in
    let isVar : Obj.t -> bool = Env.is_var e in
    if has_free_vars isVar (Obj.repr ans) then HasFreeVars (isVar, Obj.repr ans)
    else Final ans
  in
  Stream.map f ans
...
module LogicAdder =
  struct
    ...
    let succ (prev: 'a -> State.t -> 'b) (f: ('q,'w) fancy -> 'a) : State.t -> ('q,'w) fancy refiner * 'b =
...
module Bool =
  struct
    ...
    let show_groundf : groundf -> string = string_of_bool
...
module List =
  struct
    ...
    let show : ('a -> string) -> (_,_) flist -> string = fun f xs ->
      let b = Buffer.create 40 in
      let rec helper = function
      | Nil -> ()

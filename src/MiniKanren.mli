...
(* val refine_fancy: ('a, 'b) fancy -> var_checker -> ('a -> 'c logic) -> 'c logic *)
...
type 'a reification_rez = Final of 'a | HasFreeVars of ((Obj.t -> bool) * Obj.t)
(** Some type to refine a stream of states into the stream of answers (w.r.t. some known
    logic variable *)
type 'a refiner = State.t Stream.t -> 'a reification_rez Stream.t
...
val one :
  unit ->
  (('a -> State.t -> 'b) -> State.t -> 'a refiner * 'b) *
  (('c -> 'd) -> 'c -> 'd) * (('e -> ('e -> 'f) -> 'f) * ('g -> 'g))

val two :
  unit ->
  (('a -> 'b -> State.t -> 'c) -> State.t -> 'a refiner * ('b refiner * 'c)) *
  (('d -> 'e -> 'f) -> 'd * 'e -> 'f) *
  (('g -> ('g -> 'h) * ('g -> 'i) -> 'h * 'i) *
   ('j * ('k * 'l) -> ('j * 'k) * 'l))
(*
val three :
  unit ->
  ((('a, 'b) fancy -> 'c fancy -> 'e fancy -> State.t -> 'g) ->
...
val four :
  unit ->
  ((('a, 'b) fancy -> 'c fancy -> 'e fancy -> 'g fancy -> State.t -> 'i) ->
...
val five :
  unit ->
  ((('a, 'b) fancy -> 'b fancy -> 'c fancy -> 'd fancy -> 'e fancy -> State.t -> 'k) ->
   State.t ->
   'a refiner * ('b refiner * ('c refiner * ('d refiner * ('e refiner * 'k))))) *
...
val qrs :
  unit ->
  (('a -> 'c  -> 'e -> State.t -> 'g) -> State.t -> 'a refiner * ('c refiner * ('e refiner * 'g))) *
  (('h -> 'i -> 'j -> 'k) -> 'h * ('i * 'j) -> 'k) *
  (('l -> ('l -> 'm) * (('l -> 'n) * ('l -> 'o)) -> 'm * ('n * 'o)) *
   ('p * ('q * ('r * 's)) -> ('p * ('q * 'r)) * 's))
...
val pqrst :
  unit ->
  (('a -> 'b -> 'c -> 'd -> 'e -> State.t -> 'k) ->
   State.t -> 'a refiner * ('b refiner * ('c refiner * ('d refiner * ('e refiner * 'k))))) *
  (('l -> 'm -> 'n -> 'o -> 'p -> 'q) -> 'l * ('m * ('n * ('o * 'p))) -> 'q) *
  (('r ->
    ('r -> 's) * (('r -> 't) * (('r -> 'u) * (('r -> 'v) * ('r -> 'w)))) ->
    's * ('t * ('u * ('v * 'w)))) *
...
(*
(* Separated module for built-in reifiers *)
module Reifiers : sig
    (* Not yet sure it this is better than
       smth : var_checker -> ('a, 'b) fancy -> 'a logic
    *)
  type 'a t = var_checker -> 'a -> 'a logic
  val int    : int t
  val string : string t
end
*)
...
val var_of_fancy_exn : var_checker -> ('a,'b) fancy -> (var_checker -> ('a,'b) fancy -> 'b) -> 'b

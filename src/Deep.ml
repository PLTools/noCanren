    ...
    method  c_Ident :
      'inh ->
        ('inh,'a logic,'syn,< a: 'ia -> 'a -> 'sa   > ) GT.a ->
          GT.int GT.list -> GT.int -> 'syn
    method  c_Var :
      'inh ->
        ('inh,'a logic,'syn,< a: 'ia -> 'a -> 'sa   > ) GT.a ->
          GT.int GT.list -> GT.int -> 'a logic GT.list -> 'syn
    method  c_Value :
      'inh ->
        ('inh,'a logic,'syn,< a: 'ia -> 'a -> 'sa   > ) GT.a ->
...
let logic :
  ...
  let rec logic_gcata fa trans inh subj =
    ...
    and tpo = object method a = fa end
     in
    match subj with
    | Ident (p0,p1) -> trans#c_Ident inh (GT.make self subj tpo) p0 p1
    | Var (p0,p1,p2) -> trans#c_Var inh (GT.make self subj tpo) p0 p1 p2
    | Value p0 ->
  ...
  object (this)
    ...
    method c_Var inh subj p0 p1 p2 =
      ((("Var (" ^
          ...
          ^ (", " ^ (GT.lift (GT.int.GT.plugins)#show () p1)))
         ^
         (", " ^
            (GT.lift
               ((GT.list.GT.plugins)#show
                  (GT.transform logic (subj.GT.t)#a this ())) () p2)))
        ^ ")"
    method c_Ident inh subj p0 p1 =
      (("Ident (" ^
          (GT.lift
             ((GT.list.GT.plugins)#show (GT.lift (GT.int.GT.plugins)#show ()))
             () p0))
         ^ (", " ^ (GT.lift (GT.int.GT.plugins)#show () p1)))
        ^ ")"
  end
class ['a,'sa] gmap_proto_logic env =
  object (this)
    ...
    method c_Var inh subj p0 p1 p2 =
      Var
        ((GT.lift
            ((GT.list.GT.plugins)#gmap (GT.lift (GT.int.GT.plugins)#gmap ()))
            () p0), (GT.lift (GT.int.GT.plugins)#gmap () p1),
          (GT.lift
...
type ('a, 'l) llist = ('a, 'l) MiniKanrenStd.llist = Nil | Cons of 'a * 'l
...
class ['a,'sa,'l,'sl] gmap_llist_t = let self = Obj.magic (ref ())  in
  object (this)
    inherit  ['a,unit,'sa,'l,unit,'sl,unit,('sa,'sl) llist] llist_t
    inherit  ((['a,'sa,'l,'sl] gmap_proto_llist) self)
    initializer self := (this :> ('a,'sa,'l,'sl) gmap_llist_t)
  end
let llist :
  (('ia -> 'a -> 'sa) ->
     ('il -> 'l -> 'sl) ->
...
type 'a lnat = 'a MiniKanrenStd.lnat = O | S of 'a
...
class ['a] show_lnat_t = let self = Obj.magic (ref ())  in
  object (this)
    inherit  ['a,unit,string,unit,string] lnat_t
    inherit  ((['a] show_proto_lnat) self)
    initializer self := (this :> 'a show_lnat_t)
  end
class ['a,'sa] gmap_lnat_t = let self = Obj.magic (ref ())  in
  object (this)
    inherit  ['a,unit,'sa,unit,'sa lnat] lnat_t

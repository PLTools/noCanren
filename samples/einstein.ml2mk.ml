...
let any_of_person f (p1, p2, p3, p4, p5) =
...
let clue02 st =
  let for_person (c, n, _, _, _) =
    (n = Englishman) && (c = Red) in
  any_of_person for_person st


(* У испанца есть собака *)
let clue03 st =
  let for_person (_, n, _, _, p) =
    (n = Spaniard) && (p = Dog) in
  any_of_person for_person st
...
(* Украинец пьёт чай *)
let clue05 st =
  let for_person (_, n, d, _, _) =
    (n = Ukrainian) && (d = Tea) in
  any_of_person for_person st


(* Зелёный дом стоит сразу справа от белого дома *)
let clue06 st =
  let for_neighbors_pair (c1, _, _, _, _) (c2, _, _, _, _) =
    (c1 = Ivory) && (c2 = Green) in
  any_of_neighbors_pair for_neighbors_pair st


(* Тот, кто курит Old Gold, разводит улиток *)
let clue07 st =
  let for_person (_, _, _, s, p) =
    (s = Old_Gold) && (p = Snails) in
  any_of_person for_person st


(* В жёлтом доме курят Kool *)
let clue08 st =
  let for_person (c, _, _, s, _) =
    (c = Yellow) && (s = Kools) in
  any_of_person for_person st


(* В центральном доме пьют молоко *)
let clue09 (_, _, (_, _, d, _, _), _, _) =
  d = Milk

(* Норвежец живёт в первом доме *)
let clue10 ((_, n, _, _, _), _, _, _, _) =
  n = Norwegian
...
let clue11 st =
  let for_neighbors_pair (_, _, _, s1, p1) (_, _, _, s2, p2) =
    ((s1 = Chesterfield) && (p2 = Fox)) || ((p1 = Fox) && (s2 = Chesterfield)) in
  any_of_neighbors_pair for_neighbors_pair st
...
let clue12 st =
  let for_neighbors_pair (_, _, _, s1, p1) (_, _, _, s2, p2) =
    ((s1 = Kools) && (p2 = Hourse)) || ((p1 = Hourse) && (s2 = Kools)) in
  any_of_neighbors_pair for_neighbors_pair st


(* Тот, кто курит Lucky Strike, пьёт апельсиновый сок *)
let clue13 st =
  let for_person (_, _, d, s, _) =
    (s = Lacky_Strike) && (d = Orange_juice) in
  any_of_person for_person st


(* Японец курит Parliament *)
let clue14 st =
  let for_person (_, n, _, s, _) =
    (n = Japanese) && (s = Parliament) in
  any_of_person for_person st
...
let clue15 st =
  let for_neighbors_pair (c1, n1, _, _, _) (c2, n2, _, _, _) =
    ((n1 = Norwegian) && (c2 = Blue)) || ((c1 = Blue) && (n2 = Norwegian)) in
  any_of_neighbors_pair for_neighbors_pair st

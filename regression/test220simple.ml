...
let () =
  run show_int   (-1) q   ("just_a"  ,(fun q     -> prog id_set0 [just_a  ]     (invoke "just_a"    (!^ q))   )) qh;
  run show_int   (-1) q   ("a_and_b" ,(fun q     -> prog id_set0 [a_and_b ]     (invoke "a_and_b"   (!^ q))   )) qh;
  run show_int   (-1) q   ("a_and_b'",(fun q     -> prog id_set0 [a_and_b']     (invoke "a_and_b'"  (!^ q))   )) qh;
  run show_int   (10) q   ("fives"   ,(fun q     -> prog id_set0 [fives]        (invoke "fives"     (!^ q))   )) qh;
  ()

...
module Gterm = struct
  module X = struct
    type ('s, 'xs) t =
      | Symb  of 's
      | Seq   of 'xs [@@deriving show { with_path = false }]
  ...
  type rterm = (string, rterm List.ground) X.t [@@deriving show { with_path = false }]
  type lterm = (string logic, lterm List.logic) X.t logic [@@deriving show { with_path = false }]
  type fterm = (rterm, lterm) fancy

  (* let rec show_rterm : rterm -> string = fun t -> X.show pp_s (List.pp_ground show_rterm) t *)
...
module Gresult = struct
  module X = struct
    type ('s, 't, 'xs) t =
    | Closure of 's * 't * 'xs
    | Val     of 't [@@deriving show { with_path = false }]
  ...
  type rresult = (string, Gterm.rterm, (string * rresult) List.ground) X.t [@@deriving show { with_path = false }]
  type lresult = (string logic, Gterm.lterm, (string logic * lresult) logic List.logic) X.t logic [@@deriving show { with_path = false }]

...
let _ = Option.(
    run_exn show_int 1 q qh ("?",(fun q -> q === !!5));
    runR int_opt_reifier show_int_opt show_intl_optl 1 q qh ("?",(fun q -> q === some !!5));
    runR int_opt_reifier show_int_opt show_intl_optl 1 q qh ("?",(fun q -> q === none ()));
    runR MiniKanren.reify show_int     show_intl   1 q qh ("?",(fun q -> some q === some !!5 ));
    runR int_opt_reifier show_int_opt show_intl_optl 1 q qh ("?",(fun q -> call_fresh (fun w -> q === some w)))
  )
...
let _ =
  run_exn show1 1  q qh ("?",(fun q -> q === Result.ok !!5 ));
  runResult   (-1) q qh ("?",(fun q -> call_fresh (fun r -> (q === Result.ok r) &&& conde [r === !!5; success])));
  runResult   (-1) q qh ("?",(fun q -> Fresh.two (fun r s -> conde
                                                                [ (q === Result.ok    s) &&& (s =/= !!4)
                                                                ; (q === Result.error r)
                                                                ])
                        ))

...
let runB n = runR Bool.reify show_bool (GT.show Bool.logic) n
...
let runBL n = runR (List.reify Bool.reify) show_bool_list show_lbool_llist n
...
let () =
  ...
  runN                     1    q   qh (REPR (fun q     -> List.lengtho (inj_list [!!(); !!(); !!()]) q    ));

...
let show_env_logic = show(List.logic) @@ show(logic) (show(Pair.ground) (show(logic) (fun s -> s)) show_llam)

let pair c p = Pair.reify OCanren.reify glam_reifier c p

let env_reifier c xs = List.reify pair c xs

let show_env  = show(List.ground) @@ (GT.show Pair.ground)  show_string  GLam.show_rlam
let show_envl = show(List.logic ) @@ show(logic) GT.(show Pair.ground show_stringl GLam.show_llam)
